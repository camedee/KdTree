...
public class KdTree<Point extends PointI>
{
	...
	KdTree(int dim, ArrayList<Point> points, int max_depth) {
		...
		//ArrayList<Point> p=tri(points,0);
		Point e=points.get((points.size()+1)/2);
		this.root_=new KdNode(e,0);
		this.next(subList(points, 0,(points.size()+1)/2),1%this.dim_,max_depth-1);
		this.next(subList(points, (points.size()+1)/2+1,points.size()),1%this.dim_,max_depth-1);
	...
	ArrayList<Point> subList(ArrayList<Point> points, int deb, int fin){
		ArrayList<Point> res=new ArrayList<Point>();
		assert (deb<fin);
		assert (fin<points.size());
	...
	void next(ArrayList<Point> points, int d, int max_depth){
		if (max_depth>0){
			points.sort(new Comparator<Point>() {
	            public int compare(Point p1, Point p2) {
			...
			Point e=points.get((points.size()+1)/2);
			this.insert(e);
			this.next(subList(points, 0,(points.size()+1)/2), (d+1)%this.dim_, max_depth-1);
			this.next(subList(points, (points.size()+1)/2+1,points.size()),(d+1)%this.dim_,max_depth-1);
		}else{
			this.insert(bary(points));
		}
	}
	...
	/// Query Functions ///
	///////////////////////
	Point bary(ArrayList<Point> points){
	...
	ArrayList<Point> tri(ArrayList<Point> points, int d){
		ArrayList<Point> p=new ArrayList<Point>();
		for (Point i:points){
			int j=0;
			int test=i.get(d);
			while((test>p.get(j).get(d)) & (j<p.size())){
				j+=1;
			}
			p.add(j,i);
		}
